...
def readephemdata(sc_id,date,mo,day):
        ...
        ephemfileglob=glob.glob(sin_PADs.project_data_dir+'/ephemeris/'+sc_id+'/rbsp'+sc_id.lower()+'_def_MagEphem_TS04D_'+str(date)+str(mo)+str(day)+'_v*.h5')

        #file_exists=exists(sin_PADs.project_data_dir+'/ephemeris/'+sc_id+'/rbsp'+sc_id.lower()+'_def_MagEphem_TS04D_'+str(date)+str(mo)+str(day)+'_v*.h5')#
#    print('this updated!')
        #if (file_exists==False):
        if len(ephemfileglob)==0:
            os.chdir(sin_PADs.project_data_dir+'/ephemeris/'+sc_id)
...
#        os.remove(sin_PADs.project_data_dir+'/'+instrument+'/L4PAI/'+sc_id+'/rbsp'+sc_id.lower()+'_ect_'+instrument+'-PAI'+str(yr)+str(mo)+str(day)+'.cdf')
#wget -r -np -nd -nc -A ‘*.cdf’ https://spdf.gsfc.nasa.gov/pub/data/rbsp/rbspa/l2/ect/rept/sectors/rel03/2012/
        ephemfileglob=glob.glob(sin_PADs.project_data_dir+'/ephemeris/'+sc_id+'/rbsp'+sc_id.lower()+'_def_MagEphem_TS04D_'+str(date)+str(mo)+str(day)+'_v*.h5')
#https://rbsp-ect.newmexicoconsortium.org/data_pub/rbspa/MagEphem/definitive/2013/
